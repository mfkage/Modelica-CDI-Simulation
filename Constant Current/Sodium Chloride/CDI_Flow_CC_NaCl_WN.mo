model CDI_Flow_CC_NaCl_WN
  ...
  Modelica.Electrical.Analog.Sources.StepCurrent stepCurrent1(I = 0.016, startTime = 3600)  annotation(
    Placement(visible = true, transformation(origin = {-36, 140}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
equation
  connect(Cap_C.n, stepCurrent1.p) annotation(
    Line(points = {{-172, 22}, {-172, 22}, {-172, 140}, {-46, 140}, {-46, 140}}, color = {0, 0, 255}));
  connect(Cap_A.p, stepCurrent1.n) annotation(
    Line(points = {{142, 36}, {140, 36}, {140, 140}, {-26, 140}, {-26, 140}}, color = {0, 0, 255}));
  connect(resistor1.p, stepCurrent1.p) annotation(
    Line(points = {{-46, 116}, {-46, 116}, {-46, 140}, {-46, 140}}, color = {0, 0, 255}));
  connect(stepCurrent1.n, resistor1.n) annotation(
    Line(points = {{-26, 140}, {-26, 140}, {-26, 116}, {-26, 116}}, color = {0, 0, 255}));
...
//Adsorbierte Stoffmenge ohne angelegte Spannung. Intervall muss länger sein als die Adsorptionszeit
    ...
    n0_CC_Na = CC_Na.amountOfSubstance;
  end when;
  n_diff_Na = (CA_Na.amountOfSubstance - n0_CA_Na + CC_Na.amountOfSubstance - n0_CC_Na) * Cal_factor;
//[mmol]
