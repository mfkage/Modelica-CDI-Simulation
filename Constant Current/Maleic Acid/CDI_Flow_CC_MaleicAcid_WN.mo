model CDI_Flow_CC_MaleicAcid_WN
  extends Chemical;
  ...
  Modelica.Electrical.Analog.Basic.Resistor resistor1(R = 25) annotation(
    Placement(visible = true, transformation(origin = {-11, 144}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  Modelica.Electrical.Analog.Sources.StepCurrent stepCurrent1(I = 0.080, startTime = 3000) annotation(
    Placement(visible = true, transformation(origin = {-10, 160}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
equation
  connect(stepCurrent1.p, Cap_C.n) annotation(
    Line(points = {{-20, 160}, {-156, 160}, {-156, 34}, {-156, 34}}, color = {0, 0, 255}));
  connect(stepCurrent1.n, Cap_A.p) annotation(
    Line(points = {{0, 160}, {162, 160}, {162, 36}, {162, 36}}, color = {0, 0, 255}));
  connect(stepCurrent1.p, resistor1.p) annotation(
    Line(points = {{-20, 160}, {-20, 160}, {-20, 144}, {-20, 144}}, color = {0, 0, 255}));
  connect(stepCurrent1.n, resistor1.n) annotation(
    Line(points = {{0, 160}, {0, 160}, {0, 144}, {0, 144}}, color = {0, 0, 255}));
  ...
  pH = -log10(H.a);
  U_max = 1.2 + stepCurrent1.I * 2;
  SAC = (n_diff_Ma1 + n_diff_Ma2 + n_diff_Ma) / m_AK;
//[mmol/g]
  Adsorptionszeit = time - stepCurrent1.startTime + 0.0000001;
//[s] +0.0000001 weil sonst bei time = 3000 s die Adsorptionszeit 0 beträgt
  ASAR = SAC / Adsorptionszeit;
//[mmol/(g*s)]
  when stepCurrent1.v < (-U_max) then
//Wird eine bestimmte Spannung erreicht, so wird die Simulation abgebrochen
    terminate("done");
  end when;
